                  ...
                  cv::Mat &cos_phi, cv::Mat &sin_phi)
  {
    ...
    for (int y = 0; y < rows; ++y)
    {
          ...
          ++row_cos_theta, ++row_sin_theta, ++row_cos_phi, ++row_sin_phi, ++row_points, ++row_r)
      {
        ...
        float theta = (float)std::atan2(row_points->val[0], row_points->val[2]); //azimuth x/z
...
public:
    ...
    void computeMInverse(cv::Mat &cos_theta, cv::Mat &sin_theta, cv::Mat &cos_phi, cv::Mat &sin_phi)
    {
        ...
        Mat33T VVt; //todo save, M matrix in the next line
...
//                signNormal(MBr, *normal);
    ...
    /** Compute the covariance of normals
    * @param r
    * @return
    */
    virtual void
    computeCovariance(const cv::Mat &r, cv::Mat_<Mat33T> & cov_mat) const
    {
        // Compute B
//        cv::Mat_<Vec3T> B(rows_, cols_);
//
...
//        }
        ...
        cv::Mat_<Mat33T> D(rows_, cols_);
        Mat33T VVt; //todo save, M matrix in the next line
        const Vec3T * vec = V_[0];
        Mat33T * M_ptr = D[0], *M_ptr_end = M_ptr + rows_ * cols_;
        for (; M_ptr != M_ptr_end; ++vec, ++M_ptr, ++row_r)
        {
            M_ptr->all(0.0);

//            if (*row_r > 300 || *row_r == 0)
//                M_ptr->all(0.0);
//            else
            if (*row_r > 0 && *row_r < 300)
            {
//            VVt = (*vec) * vec->t() * (1.0 / pow(*row_r, 4)); //(v * v_t) / r^4
//            *M_ptr = VVt;
                *M_ptr = (*vec) * vec->t() * (1.0 / pow(*row_r, 4)); //(v * v_t) / r^4
//                if (isnan(1.0 / pow(*row_r, 4)) || isinf(1.0 / pow(*row_r, 4))) {
//                    std::cout << "*vec" << std::endl;
//                    std::cout << *vec << std::endl;
//                    std::cout << *row_r << std::endl;
//                }
            }
        }

        ///todo BorderTypes::BORDER_TRANSPARENT, error
        // Apply a box filter to D
        boxFilter(D, D, D.depth(), cv::Size(window_size_, window_size_), cv::Point(-1, -1), false);

        // compute the Minv*B products
        row_r = r.ptr < float > (0);
//        const Vec9T * B_vec = D[0];
        const Mat33T * d_mat = D[0];
        const Mat33T * M_inv = reinterpret_cast<const Mat33T *>(M_inv_.ptr(0));
        Mat33T * cov = cov_mat.ptr<Mat33T>(0);
        for (; row_r != row_r_end; ++row_r, ++d_mat, ++cov, ++M_inv) {
            if (*row_r==FLT_MAX) {
                cov->all(0.0); //todo
            } else {
                Mat33T Mr = *M_inv;
...
//                normalizedNormal(MBr, *normal);
            ...
            if ( isnan((*cov)(0,0)) ) {
                std::cout  << "*row_r " << *row_r << std::endl;
                std::cout  << *M_inv <<std::endl;
                std::cout << *d_mat <<std::endl;
//                std::cout << Mr.t() <<std::endl;}
            }
        }
    }
