    ...
    for (int i = 0; i < range_image.rows; ++i) {
        for (int j = 0; j < range_image.cols; ++j) {
            if (range_image.at<float>(i, j) < 100.0)
                output.at<uchar>(i, j) = range_image.at<float>(i, j) / 100 * 255.0;
            else
...
void RangeImage::createAndBuildTableFromRings(const pcl::PointCloud<PointXYZIRT>::Ptr &cloud)
{
    ...
    //lidar frame，v = [cos(phi)cos(theta), -cos(phi)sin(theta), sin(phi)]^T
...
void RangeImage::computeCov(const cv::Mat &r, cv::Mat_<Mat33T> & cov_mat) {
    cov_mat = cv::Mat_<Mat33T> (rows_, cols_);

    lidar_fals.computeCovariance(r, cov_mat);
}
