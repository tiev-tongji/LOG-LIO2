...
////                  const float & dx = grad_x.at<float>(rowIdn, columnIdn);
//            int valid_grad = 0;
////                  if (dx < 500.0) // including non-range value
////                      added_pt.intensity = abs(dx);
////                  else
//            {
//                int cols = rangeMat.cols;
//                float sum_grad = 0;
//                for (int j = 1; j < 4; ++j) { // 1 invalid
//                    int left = ((columnIdn - j) + cols) % cols;
//                    int right = (columnIdn + j) % cols;
//                    float r_left = rangeMat.at<float>(rowIdn, left);
//                    float r_right = rangeMat.at<float>(rowIdn, right);
//                    if (r_left < 200 && r_right < 200) {
//                        sum_grad += abs(r_left - r_right) / (j * 2);
//                        ++valid_grad;
//                    }
//                }
//                added_pt.intensity = sum_grad / valid_grad;
//            }
//
//            const float & dist = rangeMat.at<float>(rowIdn, columnIdn);
//            added_pt.intensity /= dist;
//
//            if (added_pt.intensity > roughness_max || valid_grad == 0)
//                added_pt.intensity = roughness_max;
        }
...
//    image_cols = Horizon_SCAN / point_filter_num;
    ...
    ang_res_x = 360.0/float(image_cols); // 360 / 1800 = 0.2
    if (!compute_table) {
...
//            range_image.saveLookupTable("/tmp/table", "ring32");
    ...
    rangeMat = cv::Mat(N_SCANS, image_cols, CV_32F, cv::Scalar::all(FLT_MAX));
    Prewitt_x = cv::Mat_<float>(1, 5) << -1.0, -1.0, 0, 1.0, 1.0;
    Prewitt_x /= 6.0;
}
